...
public class 零一背包问题 {
    public static void main(String[] args) {
        零一背包问题 s = new 零一背包问题();
        System.out.println(s.dpmethod(new int[]{2,4,1,3,5,3}, 10, new int[]{3,5,2,6,7,10}));
    }

    public int dpmethod(int[] weigth, int capital, int[] value) {//01背包问题的解的遍历过程是这样的
        ...
        //第一个元素
        //for(背包容量)
        //若元素要占用的容量大于背包的容量，则进行Math.max(dp[i - 1][j], dp[i - 1][j - weigth[i-1]] + value[i-1])
        //否则不能放入背包
